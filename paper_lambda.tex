...
% PRD submission: twocolumn (above) or reprint,onecolumn for editorial review
...
\text{Zenodo Digital Repository DOI: 10.5281/zenodo.21540551}}

\date{\today}


\begin{abstract}
The cosmological constant problem includes a 120-order mismatch
between misapplied Planck-scale zero-point estimates and the observed infrared
density of order $10^{-27}\,\mathrm{kg/m^3}$.
This paper records a classical scalar--tensor effective field theory for a
vacuum-response field and an explicit constitutive infrared matching condition
linking the galactic acceleration scale $a_0$ to the Hubble rate $H_0$.
We do not claim that this matching is derived by varying the action at the
Hubble radius; it embeds the known modified-Newtonian-dynamics--cosmology
coincidence in covariant language.
We also do not derive the dark-energy fraction from $a_0$ alone; that
quantity remains fixed by cosmic microwave background data.
There is no quantum field-theory loop calculation; vacuum refers to the
classical response field only.
Numerical tables report internal consistency benchmarks against published
Solar-System, galactic-rotation, and gravitational-wave constraints, not
...
The cosmological constant problem remains arguably the most profound crisis in modern
theoretical physics~\cite{Weinberg1989,Polchinski2006,Martin2012}. Within the standard framework of Quantum
Field Theory (QFT), the vacuum energy density is computed as the sum of zero-point
energies across all fields up to a fundamental cutoff scale. If the Planck mass
($M_{\text{Pl}}$) is adopted as the natural ultraviolet (UV) limit, the theoretical
vacuum energy density scales as $\rho_{\text{th}} \sim M_{\text{Pl}}^4 \sim 10^{92}%
\text{ kg/m}^3$. Conversely, cosmological measurements from the cosmic microwave
background (CMB) and distant Type Ia Supernovae place the observed dark energy density
at $\rho_{\text{obs}} \sim 10^{-26}\text{ kg/m}^3$~\cite{Planck2020,Riess1998,Perlmutter1999,Scolnic2018}.
This yields an unprecedented mismatch of approximately 120 orders of magnitude, 
frequently referred to as the cosmological constant catastrophe~\cite{Carroll2001}.

To reconcile this immense fine-tuning dilemma, standard $\Lambda$CDM cosmology
treats $\Lambda$ either as an ad-hoc geometric integration constant or introduces
dynamical scalar fields (e.g., quintessence) that require delicate, unnatural 
potentials~\cite{Zlatev1999,Peebles1988,Wetterich1988,Caldwell1998}. Alternative approaches attempt to exploit holographic bounds or modified
gravity schemes~\cite{Padmanabhan2003,tHooft1993,Susskind1995,Bousso2002,Cohen1999,Li2004,Clifton2012,Sotiriou2010,DGP2000}, yet a direct analytical bridge connecting
microscopic vacuum parameters to macroscopic cosmic parameters without fine-tuning
...
(Sec.~\ref{sec:constitutive_closure}), numerically the same content as
the long-standing MOND--$H_0$ coincidence~\cite{Milgrom1983,Famaey2012};
\item a passive $\Lambda$CDM background $\Omega_\Lambda$ fixed by CMB
input, not output from $a_0$.
\end{enumerate}
...
$\Omega_\Lambda=0.688$, and $\Omega_m=0.315$~\cite{Planck2020}.
Recent DESI baryon-acoustic-oscillation (BAO) constraints and the
classic SDSS acoustic peak~\cite{DESI2024,Eisenstein2005} are compatible
with this passive $w=-1$ anchor at percent-level precision in
$(\Omega_m h^2,\,H_0)$; they are cited as external checks, not refit
inputs to the Planck anchors listed here.
The galactic scale is $a_0=1.20\times10^{-10}\,\mathrm{m/s^2}$
(SPARC/MOND phenomenology)~\cite{Milgrom1983,Famaey2012,Sanders2002,McGaugh2016,Lelli2017}.
Local gravity uses post-Newtonian parameters
$\gamma=\beta=1$ at the field minimum, consistent with Cassini
$\gamma-1=(2.1\pm2.3)\times10^{-5}$~\cite{Will2014,Bertotti2003}.
Tensor propagation uses $v_{\rm gw}=c$, compatible with
GW170817--GRB~170817A~\cite{Abbott2017GW}.

\paragraph{Internal consistency benchmarks (not new discoveries).}
Table~\ref{tab:validation} and Sec.~\ref{sec:test_scope} compare the EFT
to Mercury perihelion, SPARC-type rotation speeds, Sirius~B redshift,
core saturation, and binary GW speed at the level of order-of-magnitude
agreement with standard benchmarks.

\paragraph{Falsifiable targets specific to this framework.}
(i)~Horizon attractor fraction $x_\star=\Omega_m/(2\alpha_s)$ from
$H_0$, $P_0$, and Gibbons--Hawking entropy \emph{without} $\Omega_\Lambda$
in the $\chi$ sector (Sec.~\ref{sec:horizon_attractor}; current
post-diction $x_\star\simeq 0.689$ vs.\ Planck $0.688$).
...
the constitutive closure as implemented; (iv) is an open numerical channel.
...
S
=
\frac{c^4}{16\pi G}\int\! d^4x\,\sqrt{-g}\,R
...
$(-,+,+,+)$, so that for a static weak field
$g_{00}\simeq-(1+2\Phi/c^2)$ and $g_{ij}\simeq(1-2\Phi/c^2)\delta_{ij}$.
With this signature, the scalar kinetic energy density is
$T_P^{00}\supset+\frac12(\partial_t P_{\rm vac})^2$ and the action term
$-\frac12 g^{\mu\nu}\nabla_\mu P\nabla_\nu P$ is \emph{positive-definite}
(no phantom branch). A $(+,-,-,-)$ convention would flip the sign of the
same physical content; all ghost-stability statements below refer to
$(-,+,+,+)$.
...
-\frac12 g^{\mu\nu}\nabla_\mu P_{\rm vac}\nabla_\nu P_{\rm vac}
-V(P_{\rm vac})
+\mathcal{L}_{\rm matter}(\psi,\tilde g_{\mu\nu})
\\
...
=
\exp\!\left[
-\frac{\lambda_v}{2c^2}(P_{\rm vac}-P_0)^2
\right],
\qquad
\alpha_A(P_{\rm vac})\equiv\frac{d\ln A}{dP_{\rm vac}}
=
...
\qquad \lambda_P>0.
\end{equation}
Variation gives the metric equation~\eqref{eq:einstein_split} with
$T_{\mu\nu}^{\rm vac}$ from Eq.~\eqref{eq:Tvac} at the $P_0$ minimum,
and the scalar equation
\begin{equation}
\label{eq:box_pvac}
\Box P_{\rm vac}-\frac{dV}{dP_{\rm vac}}
=
-4\pi G\lambda_v T_{\rm eff},
\end{equation}
with $\Box=g^{\mu\nu}\nabla_\mu\nabla_\nu$.
The effective source is fixed by variation of
$\mathcal{L}_{\rm matter}(\psi,\tilde g)$:
\begin{equation}
\label{eq:Teff_def}
T_{\rm eff}
\equiv
...
T\equiv g^{\mu\nu}T_{\mu\nu}^{\rm matter},
\end{equation}
where $T_{\mu\nu}^{\rm matter}$ is the Jordan-frame matter tensor on
$\tilde g_{\mu\nu}=A^2 g_{\mu\nu}$.
For a perfect non-relativistic fluid in the $(-,+,+,+)$ convention,
\begin{equation}
\label{eq:trace_nr}
T
=
g^{00}T_{00}+g^{ii}T_{ii}
=
-\rho_m c^2+3P
\simeq
-\rho_m c^2.
\end{equation}
To first order in $\delta P\equiv P_{\rm vac}-P_0$,
Eqs.~\eqref{eq:A_quadratic} and \eqref{eq:trace_nr} give
\begin{equation}
\label{eq:Teff_linear}
T_{\rm eff}^{(1)}
=
-\frac{\lambda_v}{c^2}\,\delta P\,T
\simeq
+\frac{\lambda_v}{c^2}\,\delta P\,\rho_m c^2,
\end{equation}
so the product $\lambda_v T_{\rm eff}$ in Eq.~\eqref{eq:box_pvac} has a
\emph{definite} sign: a positive density excess ($\rho_m>0$) and a
positive excursion $\delta P>0$ source the scalar equation
attractively toward the minimum (no sign ambiguity in the linearized
Poisson limit).
At $P_{\rm vac}=P_0$, $\alpha_A(P_0)=0$ and $T_{\rm eff}=0$, so tensor
perturbations propagate at $c$ at linear order (Test~5).
...
Eq.~\eqref{eq:master_action} reduces to GR plus a cosmological constant
($\gamma=\beta=1$; Test~1).
At galactic radii where $g_{\rm N}<a_0$, Test~2 uses the MOND benchmark
$g_{\rm eff}=\sqrt{g_{\rm N}a_0}$ (Fig.~\ref{fig:rotation}); deriving
that relation from Eq.~\eqref{eq:box_pvac} is \emph{not} shown here.
At the IR patch, Eqs.~\eqref{eq:flrw_conservation}--\eqref{eq:ir_matching}
apply.
This is a single field $P_{\rm vac}$ with regime-dependent constitutive
readouts, not three unrelated theories.
...
\subsection{Scale Separation: Local $1/r$ vs Homogeneous FLRW}
...
$a_0=1.20\times10^{-10}\,\mathrm{m/s^2}$, and
$\Omega_\Lambda=0.688$~\cite{Planck2020}, Eq.~\eqref{eq:rho_boundary}
gives
\begin{equation}
\label{eq:numbers}
\Omega_{\rm bd}=0.183,\quad
\rho_{\rm bd}=1.56\times10^{-27}\,\mathrm{kg/m^3},\quad
\rho_\Lambda=5.33\times10^{-27}\,\mathrm{kg/m^3}.
\end{equation}
...
derivation.}
\label{tab:attractor_status}
\begin{ruledtabular}
\begin{tabular}{lll}
\textbf{Component} & \textbf{Status} & \textbf{Interpretation} \\
\hline
3D bulk sector & Null test &
Volumetric PDE veto ($\sim 20\%$ overshoot) \\
Cubic $S_{\rm vac}$ & Post-diction &
$x_\star=0.689$ ($+0.17\%$ vs.\ Planck; audit in App.~\ref{app:cubic_truncation}) \\
Circularity & Removed &
$\Omega_\Lambda$ absent from $\chi$; $\zeta/\chi$ from $(P_0,\phi_0,R_{\rm crit})$ \\
EFT status & Attractor &
Falsifiable constitutive closure (not ab initio QG) \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Equation~\eqref{eq:numbers} shows that $\rho_{\rm bd}$ and
...
\subsection{Scope of Table~\ref{tab:validation}}
...
\item \textbf{Test 2 (SPARC-type curves):} the legacy suite uses the MOND
interpolation $g_{\rm eff}=\sqrt{g_{\rm N}a_0}$ as an external benchmark;
Appendix~\ref{app:mond_limit} and \texttt{test2\_sparc\_field.py} integrate
the spherical AQUAL limit of the gradient law ($\sim 4\%$ offset at
$50$\,kpc for the reference galaxy).
\item \textbf{Test 3 (Sirius~B):} Newtonian/redshift check with fixed
$\rho_{\rm loc}$; not a unique prediction of the action.
\item \textbf{Test 4 (core saturation):} the legacy suite uses
Eq.~\eqref{eq:Pvac_sat} as an EFT proxy; Appendix~\ref{app:core_from_V} and
\texttt{test4\_spherical\_core.py} integrate $P_{\rm vac}(r)$ from the
quartic $V(P)$ with derived $\ell_{\rm sat}$ (finite $K$ at the center).
\end{itemize}
...
\caption{Internal consistency benchmarks (not independent falsification).
See Sec.~\ref{sec:test_scope}.}
\label{tab:validation}
\begin{ruledtabular}
\begin{tabular}{lccc}
\textbf{Test Regime} & \textbf{Quantity} & 
\textbf{Theoretical / Observed} & \textbf{Status} \\
\hline
1. Solar Field & Perihelion & $42.98''/\text{cy}$ ($\Delta = 0.005\%$) & \text{Pass} \\
2. Galactic ($a_0$) & SPARC Curves & Flat $199.77\text{ k/s}$ ($15\text{--}50\text{k}$) & \text{Pass} \\
3. Strong Field & Sirius B & $77.05\text{ km/s}$ ($\Delta = 4.17\%$)~\cite{Shapiro2004} & \text{Pass} \\
4. Limit $r \to 0$ & Core saturation & Finite $P$ (Eq.~\eqref{eq:Pvac_sat}) & \text{Pass} \\
5. Binary / GW & LIGO Speed & $v_{\text{gw}} = c$ ($\Delta = 0.00$) & \text{Pass} \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
Boundary numbers in Eq.~\eqref{eq:numbers} and the five-test suite
(Table~\ref{tab:validation}) are reproducible from the GRAVEDAD3 tree:
\begin{verbatim}
...
\end{verbatim}
See \texttt{paper\_constante\_cosmologica/README.md} and GitHub Actions
workflow \texttt{paper-lambda-reproduce.yml}.
The first script prints $(\Omega_{\rm bd},\rho_{\rm bd},\rho_\Lambda)$;
the second executes the frozen cromatic2/GRAVEDAD3 validation bank
(Solar System, SPARC-type rotation, Sirius~B, core regularization,
GW speed).
...
Sec.~\ref{sec:horizon_attractor} (Table~\ref{tab:attractor_status}).
...
By embedding a classical vacuum-response EFT
Eq.~\eqref{eq:master_action}, the note records the UV--IR hierarchy through
the \emph{constitutive} matching $\Omega_{\rm bd}=a_0/(cH_0)$ while keeping
$\Omega_\Lambda$ as the passive CMB-fixed background.
Sec.~\ref{sec:constitutive_closure} states explicitly what is matching
versus what would be circular; Sec.~\ref{sec:test_scope} limits the
interpretation of Table~\ref{tab:validation}.
Appendices~\ref{app:cubic_truncation}--\ref{app:core_from_V} address
parametric-overfitting, 3D/2D, and benchmark-scope critiques raised in
peer review.
...
\section{Deep-field MOND limit of Eq.~\eqref{eq:box_pvac}}
\label{app:mond_limit}

Test~2 does \emph{not} claim that Eq.~\eqref{eq:box_pvac} has been
integrated over SPARC galaxies.
It only checks consistency with the standard deep-MOND interpolation
$g_{\rm eff}=\sqrt{g_{\rm N}a_0}$ used elsewhere in the literature.
A derivation path \emph{within} the EFT is nevertheless identifiable.
In the static weak-field limit with $\alpha_A(P_0)=0$,
Eq.~\eqref{eq:a0_field} links $a_0$ to the exterior gradient
$P_{\rm vac}=P_0-C/r$ via $K_g C=r_\ast^2 a_0$.
In the chameleon/thin-shell regime
(Sec.~\ref{sec:conformal_wep}), $|\delta P/P_0|\ll 1$ in the Solar
System, but at galactic radii where $g_{\rm N}\lesssim a_0$ the
nonlinear drive $-4\pi G\lambda_v T_{\rm eff}$ in
Eq.~\eqref{eq:box_pvac} competes with the gradient term.
Matching the transition radius where $K_g|\nabla P_{\rm vac}|=g_{\rm N}$
to the saturation scale $K_g|\nabla P_{\rm vac}|=a_0$ yields, in the
deep limit $g_{\rm N}\ll a_0$,
\begin{equation}
...
\sqrt{g_{\rm N}a_0},
\end{equation}
the QUMOND/deep-MOND scaling, \emph{provided} $C\propto\sqrt{M}$ in the
low-acceleration branch (standard chameleon screening).
...
external formula while keeping the benchmark scope honest.
Script \texttt{test2\_sparc\_field.py} integrates the spherical AQUAL
limit $\nabla\!\cdot(\mu(g/a_0)\mathbf{g})=4\pi G\rho$ with
$\mu(x)=x/\sqrt{1+x^2}$ (no external $\sqrt{g_Na_0}$ input)~\cite{Milgrom2009,Bekenstein2004,Skordis2006}.
For $M=10^{11}M_\odot$, $R_d=3$\,kpc, $v(50\,\mathrm{kpc})\simeq 201$\,km/s
($\sim 4\%$ mean offset from the MOND interpolation benchmark; suite band
$150$--$250$\,km/s passed).

\section{Core saturation from Eq.~\eqref{eq:Vquartic}}
