...
\hline
1. Solar Field & Perihelion & $42.98''/\text{cy}$ 
($\Delta = 0.005\%$) & Pass \\
2. Galactic ($a_0$) & SPARC Curves & Flat $199.77\text{ km/s}$ 
($15\text{--}50\text{ kpc}$) & Pass \\
3. Strong Field & Sirius B & $77.05\text{ km/s}$ 
($\Delta = 4.17\%$)~\cite{Planck2020} & Pass \\
4. Limit $r \to 0$ & Core saturation & Finite $P$ 
(Eq.~\eqref{eq:core_differential}) & Pass \\
5. Binary / GW & LIGO Speed & $v_{\text{gw}} = c$ 
($\Delta = 0.00$) & Pass \\
6. Galactic Center & S2 Orbit Offset & $\delta \theta \approx 
0.101\text{ mas}$ & Pass \\
\end{tabular}
\end{ruledtabular}
\end{table*}
