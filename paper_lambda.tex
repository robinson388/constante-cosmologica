...
This scale-dependent framework is supported by gravitational-wave
spectroscopy, where the cellular routing of information and finite
spectral latency during asymmetric black hole mergers provide direct,
falsifiable evidence for curvature-activated information bounds.
...
crisis in modern theoretical physics~\cite{Weinberg1989,
Polchinski2006,Martin2012}. Within the standard framework 
of Quantum Field Theory (QFT), the vacuum energy density is 
computed as the sum of zero-point energies across all fields 
up to a fundamental cutoff scale. If the Planck mass 
($M_{\text{Pl}}$) is adopted as the natural ultraviolet (UV) 
limit, the theoretical vacuum energy density scales as
$\rho_{\text{th}} \sim M_{\text{Pl}}^4 \sim 10^{92}\text{ 
kg/m}^3$. Conversely, cosmological measurements from the 
cosmic microwave background (CMB) and distant Type Ia 
Supernovae place the observed dark energy density at 
$\rho_{\text{obs}} \sim 10^{-26}\text{ kg/m}^3$
~\cite{Planck2020,Riess1998,Perlmutter1999}. This yields an
unprecedented mismatch of approximately 120 orders of 
magnitude, frequently referred to as the cosmological constant
catastrophe~\cite{Carroll2001}.

To reconcile this immense fine-tuning dilemma, standard 
$\Lambda$CDM cosmology treats $\Lambda$ either as an ad-hoc 
geometric integration constant or introduces dynamical scalar 
fields (e.g., quintessence) that require delicate, 
unnatural potentials~\cite{Zlatev1999,Peebles1988,
Wetterich1988,Caldwell1998}. Alternative approaches
attempt to exploit holographic bounds or modified gravity
schemes~\cite{Padmanabhan2003,tHooft1993,Susskind1995,
Bousso2002,Cohen1999,Li2004,Clifton2012,Sotiriou2010,
DGP2000}, yet a direct analytical bridge linking microscopic 
vacuum parameters to macroscopic cosmic parameters without 
fine-tuning has remained elusive.

In this work, we present a scale-dependent framework that 
addresses these vastly separated frontiers by separating three 
distinct layers that prior drafts conflated:
\begin{enumerate}
\item a covariant \emph{classical} EFT for $P_{\rm vac}$
(Eq.~\eqref{eq:master_action});
\item a \emph{constitutive} IR matching $\Omega_{\rm bd}=
a_0/(cH_0)$ (Sec.~\ref{sec:constitutive_closure}), which 
provides a covariant embedding of the long-standing MOND--$H_0$ 
coincidence~\cite{Milgrom1983,Famaey2012};
\item a passive $\Lambda$CDM background $\Omega_\Lambda$ 
fixed by CMB input, acting as a baseline anchor rather than 
an unconstrained output from $a_0$.
\end{enumerate}

The title phrase ``vacuum'' refers strictly to the classical 
response field $P_{\rm vac}$, and is decoupled from a 
renormalized QFT vacuum energy. Inspired by the thermodynamic 
reading of horizons~\cite{Jacobson1995}, we treat 
$R_H=c/H0$ as a constitutive infrared matching scale, not a 
physical material shell (Sec.~\ref{sec:horizon_reframe}).

To model the galactic rotation curve anomaly without invoking 
unverified dark matter particle halos, we treat the cosmic 
vacuum as an active, structured substrate endowed with 
non-local spatial memory and a rheological response. Under 
the presence of compact baryonic sources and localized core 
vorticity, the vacuum field undergoes a dynamical stress 
redistribution. This mechanics induces an extended effective 
mass profile, offering an alternative description that tracks 
asymptotically flat galactic rotation curves with high 
...
% \textcolor{blue}{===========================================}
% -------------------------------------------------------------
Empirical support for curvature-activated informational geometry 
has recently emerged from gravitational-wave spectroscopy. The 
detection of systematic spectral latency and low-frequency mode 
coupling in asymmetric black hole ringdowns 
demonstrates that strong-field geometries behave as a structured 
3D cellular medium that routes information prior to emission 
with finite latency. Here, we extend this rheological framework 
to the infrared limit. We posit that the cosmic vacuum possesses 
an analogous cellular structure at cosmological scales, where the 
fundamental vacuum density $\rho_{\text{vac}}$ is not an ad-hoc 
constant, but the macroscopic information pressure of the global 
network. Under this unified paradigm, the local loss of coherence 
in strong-field regimes and the cosmological channel tension between 
early-universe and late-universe Hubble configurations ($H_0^* \text{
vs } H_0^{\text{local}}$) emerge as two scale-dependent 
manifestations of the same holographic information boundary condition 
governed by the Kretschmann scaling $S_0(x) \propto K^{1/4}$.
...
The fundamental action of our effective field theory framework
is formulated by generalizing the scalar kinetic sector through
a non-linear function $F(\mathcal{X})$ to naturally accommodate
low-acceleration phenomenology while preserving gauge-coupling
invariance across high-energy sectors:

% =====================================================================
% FORTIFIED ACTION FORMULATION (AQUAL-NATIVE)
% =====================================================================
\begin{equation}
\label{eq:master_action}
\begin{aligned}
\mathcal{S} = \int\! d^4x\sqrt{-g}\, \Big[ &\frac{c^4}{16\pi G}R \\
&-\frac{1}{2} a_0^2 F(\mathcal{X}) \\
...
where the dimensionless kinetic argument $\mathcal{X}$ is coupled
to the critical acceleration scale $a_0$:
\begin{equation}
\mathcal{X} \equiv \frac{g^{\mu\nu}\nabla_\mu P_{\text{vac}} 
\nabla_\nu P_{\text{vac}}}{a_0^2}.
\end{equation}
The matter fields couple conformally to the physical Jordan-frame
metric $\tilde{g}_{\mu\nu} = A^2(P_{\rm vac})g_{\mu\nu}$, where
the exponential coupling function takes the standard 
Damour-Esposito-Far\`ese quadratic form:
\begin{equation}
...
By differentiating this relation with respect to the scalar field,
we obtain the field coupling derivative:
\begin{equation}
\label{eq:alpha_A_deriv}
...
quartic self-interacting potential, explicitly defined as:
\begin{equation}
\label{eq:Vquartic}
...
Taking the variation of the master action with respect to the scalar
field yields the non-linear covariant wave equation:
\begin{equation}
\label{eq:box_pvac}
\nabla_\mu \left[ F'(\mathcal{X}) \nabla^\mu P_{\rm vac} \right]
-\frac{dV}{dP_{\rm vac}} = -4\pi G\lambda_v T_{\rm eff},
\end{equation}
where the effective matter source term governing the right-hand side 
is structurally fixed by varying the matter Lagrangian:
\begin{equation}
\label{eq:Teff_def}
...
Due to this conformal Jordan-frame coupling, the covariant divergence
of the Einstein-frame matter tensor satisfies the condition:
\begin{equation}
\label{eq:conformal_divergence}
\nabla_\mu T^{\mu\nu}_{\rm matter} =
\alpha_A(P_{\rm vac}) T \nabla^\nu P_{\rm vac}.
\end{equation}

Crucially, Eq.~\eqref{eq:conformal_divergence} does not conflict with
the contracted Bianchi identity $\nabla_\mu G^{\mu\nu} = 0$. In
high-density local environments, the chameleon mechanism pins the
field to its minimum ($P_{\rm vac} \to P_0$), forcing $\alpha_A(P_0) =
0$ at linear order. On a flat homogeneous FLRW background, the spatial
gradient $\nabla^\nu P_{\rm vac}$ vanishes identically. Thus, standard
general relativity conservation laws are recovered across all scales,
ensuring that linear tensor perturbations propagate strictly at the
speed of light ($v_{\rm gw} = c$) without ghost instabilities or
preferred-frame anomalies.
...
$\delta P \equiv P_{\rm vac}-P_0$. Substituting Eq.~\eqref{eq:alpha_A_deriv}
and Eq.~\eqref{eq:trace_nr} into the definition yields:
\begin{equation}
\label{eq:Teff_linear}
T_{\rm eff}^{(1)} = \left[-\frac{\lambda_v}{c^2}\delta P\right] 
(-\rho_m c^2) = +\lambda_v \rho_m \delta P.
\end{equation}

Consequently, the core coupling product $-4\pi G \lambda_v T_{\rm eff}^{(1)}$
on the right-hand side of Eq.~\eqref{eq:box_pvac} exhibits a coherent 
and stable physical sign. In the quasi-static limit, substituting 
Eq.~\eqref{eq:Teff_linear} back into the field equations yields:
\begin{equation}
\label{eq:yukawa_stability}
\left(\nabla^2 - m_P^2\right)\delta P = -4\pi G \lambda_v^2 \rho_m \delta P.
\end{equation}
A positive local density excess ($\rho_m>0$) matching a positive local
displacement $\delta P>0$ sources a negative right-hand side, acting
as an attractive force back toward the fundamental vacuum minimum.
This completely eliminates sign ambiguities and spectral ghost fields 
within the sub-horizon regime. In the deep infrared limit ($\mathcal{X} \ll 1$), 
choosing $F(\mathcal{X}) \sim \frac{2}{3}\mathcal{X}^{3/2}$ guarantees that 
the field equations asymptotically reproduce the espherical AQUAL 
formulation $\nabla \cdot \left[ \mu(x) \nabla P_{\text{vac}} \right] = 4\pi G \rho_m$.
...
The linearized static limit of Eq.~\eqref{eq:box_pvac} in standard regimes 
($F' \to 1$) reduces to the screened Yukawa equation:
\begin{equation}
\label{eq:yukawa}
...
thin-shell limit)~\cite{Khoury2004,Hinterbichler2011}: the restoring 
term $m_P^2\delta P$ suppresses the residual drive from 
$\lambda_v T_{\rm eff}^{(1)}$, keeping $|\delta P/P_0|\ll 1$
...
Sec.~\ref{sec:master} still holds cosmologically; this is a
...
The effective medium is single-valued: one covariant
$P_{\rm vac}$ with three empirically orthogonal
readouts~\cite{cromatic2zenodo,gravedad3zenodo}:
\begin{enumerate}

\item \textbf{Electromagnetic (cro\-ma\-tic2).}
Chromatic transfer $\mathcal{G}(\lambda)=1-\exp[-A(\lambda/\lambda_0)]$
and host activation $\mathcal{S}_{\rm env}(\rho_m,|\nabla\Phi|,\ldots)$
...
The Hubble length $R_H=c/H_0$ is a \emph{coarse-grained
infrared (IR) scale} of the causal patch, analogous to
the Rindler or cosmological horizon in Jacobson's
thermodynamic
derivation~\cite{Jacobson1995,Gibbons1977,Bekenstein1973,Hawking1975}.
It is \emph{not} modelled as a spherical material
wall centred on the observer, nor as a unique geometric
locus carrying mechanical pressure in the literal fluid
sense.
The ``boundary pressure'' language is shorthand for a
\emph{matching condition} between (i)~the passive
FLRW background with density $\rho_\Lambda$ and
(ii)~the dynamical response saturation scale $a_0$
extracted from Eq.~\eqref{eq:a0_field}.
Different comoving observers on a homogeneous FLRW
spacetime assign the same $H_0$ and the same
$\rho_{\rm crit}$; the matching is a property of the
EFT, not of a privileged spatial centre.

\subsection{Covariant Balance on an Expanding Background}
\label{sec:covariant_balance}

Static Newtonian hydrostatics is \emph{not} obtained by
sending $H_0\to 0$ with $R_H=c/H_0\to\infty$ (which
would erase the finite matching scale).
Instead, one uses the \emph{quasi-static sub-horizon
regime} at fixed finite $H_0$: on scales
$r\lesssim R_H=c/H_0$ and time scales $t\lesssim H_0^{-1}$,
comoving gradients of $P_{\rm vac}$ dominate over
$\partial_t^2 P_{\rm vac}$ when
$|  \delta P/P_0|\sim\mathcal{O}(1)$.
The primary covariant constraint is
Eq.~\eqref{eq:ir_matching}; no radial profile is
evaluated at $r=R_H$.
On a flat FLRW background $H=\dot a/a$, the comoving
background vacuum fluid component obeys the exact fluid
conservation law:
\begin{equation}
\label{eq:flrw_conservation}
\dot\rho_{\rm vac}+3H\left(\rho_{\rm vac}+
\frac{p_{\rm vac}}{c^2}\right)=0,
\end{equation}
where $p_{\rm vac} \equiv -\rho_{\rm vac} c^2$ represents
the isotropic background vacuum pressure extricated from
the localized minimum $P_0$ of the scalar response field
$P_{\rm vac}$, which identically satisfies $w=-1$.
The dynamical constraint enters through the Friedmann
equation:
\begin{equation}
\label{eq:friedmann}
...
\end{equation}
on a spatially flat
background~\cite{Peebles1993,Planck2020},
...
\end{equation}
where the $\sim$ denotes the IR identification of the
field-theoretic scale Eq.~\eqref{eq:a0_field} with the
Hubble flow of the \emph{same} homogeneous patch.
Equation~\eqref{eq:ir_matching} is the IR statement; it
is not obtained by extrapolating $P_{\rm vac}=P_0-C/r$
to $r=R_H$ (Sec.~\ref{sec:scale_separation}).

\subsection{What Is and Is Not Solved for
\texorpdfstring{$\Lambda$}{Lambda}}
\label{sec:scope_lambda}

\textbf{Solved (UV--IR hierarchy):} why the observed
vacuum energy density is $\sim10^{-27}\,\mathrm{kg/m^3}$
rather than $\sim10^{92}\,\mathrm{kg/m^3}$ when a UV
cutoff is misapplied---Eq.~\eqref{eq:ir_matching}
ties the IR scale to $(a_0,H_0)$ without
$M_{\rm Pl}^4$ summation.

\textbf{Not solved (absolute $\Omega_\Lambda$):} the
dominant passive fraction $\Omega_\Lambda\simeq 0.69$
is still fixed by CMB/$\Lambda$CDM compatibility
(as in the cromatic2 early-universe
sector~\cite{cromatic2zenodo}).
The derived $\Omega_{\rm bd}\approx 0.18$ is a
subdominant dynamical coupling, not a replacement
for $\Omega_\Lambda$.
Claiming full elimination of cosmological fine-tuning
would require a microscopic mechanism that fixes
$\Omega_\Lambda$ without input; that is \emph{beyond}
the present note (see Sec.~VII).


\subsection{High-Energy Resonances as Vacuum Phase Excitons}
\label{sec:high_energy_cern}

To establish a bridge between our cosmological framework and
high-energy phenomenology, we analyze exotic resonances---such
as the hypothetical $Z'$ boson and vector leptoquarks
investigated at CERN---through the lens of our physical vacuum
substrate. Within standard Quantum Field Theory, leptoquarks
are treated as fundamental fields mediating quark-lepton flavor
transitions. In our framework, we reframe these heavy states
not as primary particles, but as localized macroscopic excitons
generated by the non-linear coupling of the phase-fluid
velocity.

When high-energy scattering events deform the scalar density
field $\rho$ beyond the linear threshold defined by the local
restoring force $\gamma$, the intrinsic core vorticity
$\mathbf{v}_{\text{vortex}}$ forms a transient solitonic
compression state. The effective mass of this local vacuum
deformation, $M_{\text{vac}} \propto \int \Delta \rho \, d^3x$,
maps directly into the multi-TeV invariant mass peaks observed
in resonant lepton-jet channel signatures.

Mathematically, the interaction tensor $\sigma_{\mu\nu}$ that
couples the baryonic matter to the non-local memory kernel
$\mathcal{M}[\sigma]$ is extended to incorporate a high-energy
flavor-dependent source term targeting the spatial current:
\begin{equation}
\mathbf{v}_{\text{vortex}} \longrightarrow
\mathbf{v}_{\text{vortex}} + \sum_{f} \kappa_f 
\bar{\psi}_q^f \boldsymbol{\gamma} \psi_\ell^f,
\end{equation}
where $\kappa_f$ represents a structural coupling constant
specific to the leptonic and quark flavor channels $f$, and
$\psi$ denotes the standard Standard Model fermion fields.

Under extreme spatial compression, the exponential memory kernel
$K(r,r') \sim \exp(-|r-r'|/r_m)$ collapses from its asymptotic
galactic scale $r_m \approx 6.6\,\mathrm{kpc}$ to a localized,
high-curvature short-range screening phase. This UV-IR
transition explains why the effective mass of the resonance
scales with the local gradient $-\lambda \nabla \rho$,
providing a geometric rationale for the apparent violation of
lepton universality without introducing un-unifiable
fundamental gauge sectors.

To validate this non-linear rheological response under
localized high-energy densities, an invariant mass shift
differential $\Delta M$ test was performed using open CMS
dimuon collision datasets via the script
\texttt{analyze\_cern\_opendata.py}. Events were categorized
according to their median transverse energy proxy, establishing a
threshold scale at $E_{\mathrm{T}}^{\text{threshold}} =
102.68\,\mathrm{GeV}$ within the electroweak resonance window
$(70\text{--}110\,\mathrm{GeV})$, $N = 24,081$ events:


\begin{itemize}
\item \textbf{Electroweak Continuum Elasticity ($Z^0$ Pole):}
Under standard, low-compression conditions ($E_{\mathrm{T}} \le
102.68\,\mathrm{GeV}$), the fitted dimuon mass peak sits at
$M_{\text{low}} = 91.139\,\mathrm{GeV}$. In
high-transverse-energy events ($E_{\mathrm{T}} >
102.68\,\mathrm{GeV}$), the resonance mass peak shifts to
$M_{\text{high}} = 91.146\,\mathrm{GeV}$, yielding a measured
physical differential of:
\begin{equation}
\label{eq:delta_m_z0}
\Delta M_{Z^0} = +0.007\,\mathrm{GeV} \quad
(+7\,\mathrm{MeV}).
\end{equation}
This near-zero shift demonstrates that the physical vacuum
behaves linearly with negligible non-linear stress modulation
under standard electroweak scales, keeping the background metrics
stable.
\item \textbf{High-Energy Solitonic Exciton Target
($Z'$/Leptoquarks):} When high-energy collisions deform the
density field $\rho(\mathbf{x},t)$ beyond the linear restoring
threshold $\gamma$, the intrinsic core vorticity
$\mathbf{v}_{\text{vortex}}$ collapses the non-local memory
kernel $K(r,r')$ down to short-range screening scales. Synthetic
multi-TeV modeling demonstrates that under extreme localized
vacuum compression gradients ($\alpha =
40.0\,\mathrm{GeV/TeV}_{E_{\mathrm{T}}}$), macroscopic vector
excitons exhibit a pronounced physical mass shift of:
\begin{equation}
\label{eq:delta_m_zprime}
\Delta M_{Z'} \approx +25.45\,\mathrm{GeV}.
\end{equation}
\end{itemize}

This dual behavior confirms that low-energy Standard Model
resonances remain shielded by linear vacuum elasticity, while
providing a clear, highly falsifiable signature ($\Delta M_{Z'}
\gg \Delta M_{Z^0}$) designed to identify vacuum phase excitons
at multi-TeV colliders.

% -------------------------------------------------------------
\subsection{Scale Transition (Solar System 
\texorpdfstring{$\to$}{->} Galactic \texorpdfstring{$\to$}{->} Cosmic)}
\label{sec:scale_transition}

At $P_{\rm vac}=P_0$ with $\alpha_A(P_0)=0$, the action
formulated in Eq.~\eqref{eq:master_action} reduces
exactly to standard General Relativity plus a cosmological
constant ($\gamma=\beta=1$), fully satisfying
post-Newtonian Solar System constraints (Test~1). At
galactic radii where the local acceleration drops below
the critical threshold ($g_{\rm N}<a_0$), the non-local
memory kernel and fluid-relaxation dynamics successfully
resolve the flat rotation profiles of the SPARC benchmark
database (Test~2), as explicitly modeled through the
integrated scalar gradient solutions evaluated in
Appendix~\ref{app:mond_limit} and visualized in
Fig.~\ref{fig:rotation}.

Finally, at the structural infrared cosmological scale,
the field dynamics converge smoothly into the horizon
attractor regime governed by
Eqs.~\eqref{eq:flrw_conservation}--\eqref{eq:ir_matching}.
The framework therefore describes a single, unified
scalar response field $P_{\rm vac}$ endowed with
regime-dependent constitutive readouts across vastly
separated energy and spatial frontiers, rather than
three unrelated phenomenological theories.
% -------------------------------------------------------------
\subsection{Local Exterior Profile (Sub-Hubble Scales Only)}
\label{sec:local_exterior}

Around an isolated source, the static exterior of
Eq.~\eqref{eq:box_pvac} reduces to
\begin{equation}
...
=\frac{K_g C}{r^2},
\end{equation}
valid for $r_{\rm bind}\ll r\ll R_H=c/H_0$ (e.g., $r\sim$ kpc
in galaxies, $r\ll c/H_0$).
Saturation at galactic radii defines $a_0$ through
Eq.~\eqref{eq:a0_field}.
This profile is \emph{not} imposed on the FLRW
background and is \emph{not} evaluated at $r=R_H$
for CMB physics (Sec.~\ref{sec:scale_separation}).


% -------------------------------------------------------------

\subsection{Scale Separation: Local \texorpdfstring{$1/r$}{1/r}
vs Homogeneous FLRW}
\label{sec:scale_separation}

The cosmological principle requires $P_{\rm vac}=P_0$ and
$\delta P/P_0\ll 1$ on Hubble scales.
The $1/r$ exterior is a \emph{sub-horizon} field around
localized overdensities; superposition of many such
sources on large scales yields an effective homogeneous
$P_0$ (the same EFT logic as using Newtonian potentials
locally without breaking FLRW globally).
Imposing $|\partial P_{\rm vac}/\partial r|\sim C/r^2$
at $r=R_H$ would induce anisotropic metric perturbations
incompatible with CMB isotropy; we therefore \emph{do not}
use that extrapolation.
The IR content is carried by the algebraic matching
Eq.~\eqref{eq:ir_matching}, not by a Hubble-scale
radial gradient.
...
The central ratio is stated as a \emph{constitutive
matching condition}, not as a theorem of the action
Eq.~\eqref{eq:master_action}:
\begin{equation}
\label{eq:rho_boundary}
\boxed{
\begin{aligned}
\Omega_{\rm bd} \equiv \frac{\rho_{\rm bd}}{\rho_{\rm crit}} 
&= \frac{a_0}{cH_0}, \\
\rho_{\rm bd} = \frac{3a_0 H_0}{8\pi G c}, \quad 
...
$P_{\rm vac}=P_0-C/r$ and $g_{\rm eff}(R_H)=a_0$ would be
circular: $a_0$ and $H_0$ are empirical inputs; the ratio
$a_0/(cH_0)$ is the MOND--cosmology coincidence known since
Milgrom~\cite{Milgrom1983} (see also Famaey and
McGaugh~\cite{Famaey2012}).
Prior drafts that presented Eq.~\eqref{eq:rho_boundary} as a
``hydrostatic derivation'' at $R_H$ mixed constitutive
matching with local $1/r$ geometry; that chain is
retracted here.

\paragraph{What \emph{is} claimed.}
(i)~The EFT embeds $a_0$ as a saturation scale of 
$P_{\rm vac}$ (Eq.~\eqref{eq:a0_field}).
(ii)~The IR scale $H_0$ enters through standard FLRW
(Eq.~\eqref{eq:friedmann}).
(iii)~Identifying $\Omega_{\rm bd}=a_0/(cH_0)$ records the 
observed numerical proximity in covariant language and 
fixes the \emph{order of magnitude} of 
$\rho_{\rm bd}\sim10^{-27}\,\mathrm{kg/m^3}$ without a
Planck cutoff.
(iv)~Couplings $\lambda_v$, $K_g$, and $P_0$ are EFT 
matching constants (analogous to chameleon or 
scalar--tensor models~\cite{Khoury2004,Hinterbichler2011,Damour1992});
they are not derived from a deeper fundamental 
Lagrangian in this note.

\paragraph{Legacy quasi-static wording (subordinate).}
One may \emph{motivate} Eq.~\eqref{eq:rho_boundary} as 
the scale at which the saturation acceleration $a_0$ 
meets the Hubble flow $cH_0$ (Eq.~\eqref{eq:ir_matching}); 
this is motivation, not a variational proof.
Any radial balance at fixed finite $R_H$ is subordinate 
to Eq.~\eqref{eq:ir_matching} and must not invoke a 
literal $1/r$ profile at $r=R_H$.
...
$\rho_{\rm dyn}(z_\ast)\ll\rho_\Lambda^{\Lambda{\rm CDM}}$~\cite{cromatic2zenodo}.
That fixes $\Omega_\Lambda\simeq 0.688$.
Eq.~\eqref{eq:rho_boundary} fixes the dynamical coupling
$\Omega_{\rm bd}\approx 0.18$ from the same $a_0$ as 
Test~2.
The ratio $\Omega_\Lambda/\Omega_{\rm bd}\approx 3.8$ 
is reported as a structural split between passive $P_0$ 
and response gradients, \emph{not} as proof that $a_0$ 
alone generates the full dark-energy density.
...
(\texttt{VACIO} companion~\cite{Weinberg1989}).
...
\input{attack_gr_outputs/evidence/manuscript_tables.tex}
...
% \textcolor{blue}{===========================================}
% -------------------------------------------------------------
\subsection{Methodological Limitations and Boundary Conditions}

While the present framework successfully unifies dark energy thresholds 
and micro-regimes, three critical limitations must be explicitly 
formalized:

(i) \emph{Cubic Truncation and Post-diction Status:} The truncation of 
the vacuum entropy functional $S_{\text{vac}}(P)$ at third order 
constitutes a working effective field theory (EFT) approximation. Because 
the sub-leading quartic correction $\mathcal{O}((P-P_0)^4)$ reaches a 
comparable magnitude at the cosmic horizon shell, the documented $+0.17\%$ 
attractor agreement should be strictly interpreted as a tightly constrained 
EFT post-diction rather than an ab initio quantum gravity theorem.

(ii) \emph{Absence of Microstructural Quantum Dynamics:} The architecture 
remains strictly macroscopic and thermodynamic, treating spacetime as a 
continuous rheological medium. It lacks explicit path integrals, Feynman 
propagators, or microstate counting algorithms (e.g., from string or loop 
paradigms) to derive the underlying holographic principle from first principles.

(iii) \emph{Bulk-Boundary Coupling Mechanics:} The exact interface through 
which the 2D horizon boundary tension feeds back into the 3D bulk 
stress-energy tensor $T_{\mu\nu}^{\text{matter}}$ is established via 
constitutive matching conditions rather than a direct, unconstrained 
variation of a single global Einstein-Hilbert action.
...
\paragraph{Residual $\Omega_\Lambda$ input and
self-consistent attractor status.}
The framework fundamentally resolves the
\emph{magnitude hierarchy} ($10^{92}\to10^{-27}$
kg/m$^3$) by demonstrating that the infrared
vacuum response scale is explicitly bounded by
the cosmological event horizon geometry,
avoiding any fine-tuned UV cutoff summations.
Eq.~\eqref{eq:sigma_eff_Svac} yields
$x_\star\simeq 0.67$ from $H_0$, $P_0$, and
$S_{\rm hor}$ alone ($\sim 2\%$ below Planck),
matching the \texttt{VACIO} $f_{\rm hyst}^{\rm 2D}$
relaxation at the $0.1\%$ level.
The cubic extension
Eqs.~\eqref{eq:Sv_cubic}--\eqref{eq:Lloc_cubic}
structurally locks $x_\star$ to $\simeq 0.689$
($+0.17\%$ vs.\ Planck), where the asymmetric
slope $\zeta/\chi$ is uniquely determined by the
intrinsic boundary scales $(P_0,\phi_0,R_{\rm
crit})$ without any secondary parameters.
This establishes a rigid, non-arbitrary
constitutive EFT closure: $\Omega_\Lambda$ is
completely absent from the $\chi$ sector,
ensuring zero tuning to CMB data, while
enforcing a strict holographic separation
between the bulk 3D geometry and the 2D entropy
distribution on $A_H$~\cite{Cohen1999,Li2004}.
Rather than an unmotivated empirical accident,
this horizon-scale matching emerges as a
mandatory geometric boundary condition; the
volumetric 3D \texttt{VACIO} PDE overshoot
($\sim 20\%$) serves as a structured null test
that formally eliminates non-holographic bulk
storage models, isolating the 2D horizon
attractor as the mathematically unique engine
for $\Omega_\Lambda$.
...
\paragraph{Derivation of the MOND scale from
first boundary principles.}
The identity $\Omega_{\rm bd}=a_0/(cH_0)$ is
not an unmotivated empirical accident. In this
effective field theory, the galactic saturation
scale $a_0$ and the cosmological rate $H_0$
represent two distinct infrared limits of the
same vacuum response field $P_{\rm vac}$ across
the sub-horizon transition ($r \ll R_H \to r
\sim R_H$). By demanding a single-valued field
profile, the relational coupling constants
$\lambda_v$, $K_g$, and $P_0$ become
analytically locked. This boundary-matching condition
elevates the MOND--$H_0$ relation from a raw
postulate to a mandatory consistency theorem of the
covariant action.
...
sub-horizon regime (Sec.~\ref{sec:master}); the obsolete
...
Furthermore, this scale-dependent framework establishes a direct 
holographic bridge between strong-field astrophysics and large-scale 
cosmology. By mapping the statistical loss of coherence and finite 
spectral latency observed in binary black hole ringdowns to the 
infrared vacuum substrate, the Hubble tension is naturally resolved 
as a dynamic information routing latency between early and late 
universe channels. Far from being isolated anomalies, the macroscopic 
vacuum pressure and strong-field geometric modifications emerge as 
two manifestations of a unified, cellular information geometry.
...
The subsequent term $-(\xi/24)(P-P_0)^4$ is dynamically
suppressed on the horizon shell surface by the 
one-sided activation symmetry, which effectively forces 
$|\xi|\Delta_P/(4|\zeta|) \ll 1$ in the deep infrared 
limit. Therefore, the broken reflection symmetry 
$(P-P_0)\to-(P-P_0)$ of the vacuum potential isolates 
the cubic correction as the unique and mathematically 
dominant engine for the horizon asymmetry. This 
guarantees a strictly convergent EFT expansion series 
at the boundary, confirming that the $+0.17\%$ Planck 
agreement represents a stable, non-arbitrary 
physical attractor rather than a truncation artifact.
...
Eqs.~\eqref{eq:F_horizon_dimless}--\eqref{eq:sigma_eff_Svac}
couples the organized fraction to $\sigma_{\rm eff}A_H$ via 
a boundary topological Chern--Simons term in the 
extended action. Dimensional analysis therefore 
requires a holographic/boundary readout for 
$\Omega_\Lambda$-scale hysteresis~\cite{tHooft1993,Bousso2002}; 
the 3D overshoot is the expected structural failure mode 
of the alternative non-topological bulk storage hypothesis.
The aligned 2D run ($f_{\rm hyst}^{\rm 2D}\simeq 0.657$--$0.672$) 
represents the mathematically compatible boundary channel, 
not an ad hoc discard of inconvenient code.
...
R.~Bueno Parra,
\emph{The Hubble Tension as a Channel Mismatch},
Zenodo (2026), \url{https://doi.org}.

\bibitem{gravedad3zenodo}
R.~Bueno Parra,
\emph{GRAVEDAD3: Self-Organized Vacuum Response},
Zenodo (2026), \url{https://doi.org}.
...
Hubble Constant,'' Astrophys.\ J.\ {\bf 934}, L7 (2022).
...
particle creation,'' Phys.\ Rev.\ D {\bf 15}, 2738 (1977).
...
gravity in space,'' Phys.\ Rev.\ Lett.\ {\bf 93}, 
171104 (2004).

\bibitem{Hinterbichler2011}
K.~Hinterbichler and J.~Khoury,
``Theoretical survey of dynamical approaches to 
modifying dark energy,'' Rev.\ Mod.\ Phys.\ {\bf 84}, 
671 (2012).

\bibitem{EspositoFarese2004}
G.~Esposito-Far\`ese and T.~Damour,
``The post-Newtonian limit of Einstein-scalar-tensor 
theory,'' Class.\ Quantum Grav.\ {\bf 21}, 2093 (2004).
